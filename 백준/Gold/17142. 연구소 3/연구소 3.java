...
public class Main{
    ...
    private static void copy(int[][] map, int[][] newMap, int[] actvirus) {
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                newMap[i][j]=map[i][j];
                if(newMap[i][j]==1) newMap[i][j]=-1;    //벽
          //      else if(newMap[i][j]==2&&!isSelect(i,j,actvirus))    newMap[i][j]=-2;   //비활성화 바이러스
            }
        }
    }

    private static boolean isSelect(int r, int c, int[] actvirus) {
        for (int i = 0; i < M; i++) {
            Pos p = virus[actvirus[i]];
